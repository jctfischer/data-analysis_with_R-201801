...
#20 # Fa�a um gr�fico dos pedidos de banana da atividade 19. O gr�fico deve ter o dia da semana no eixo X, a hora do dia no eixo Y, 
# e pontos na intersec��o dos eixos, onde o tamanho do ponto � determinado pela quantidade m�dia de pedidos de banana 
# nesta combina��o de dia da semana com hora
...
#21 # Fa�a um histograma da quantidade m�dia calculada na atividade 19, facetado por dia da semana
insta_orders %>%
  inner_join(insta_products, by = 'order_id') %>%
  filter(product_id %in%  bananas) %>%
  group_by(order_dow, order_hour_of_day) %>%
...
#22 # Teste se h� diferen�a nas vendas por hora entre os dias 3 e 4 usando o teste de wilcoxon e utilizando a simula��o da aula de testes
insta_orders %>%
  group_by(order_dow, order_hour_of_day) %>%
  summarise(count = n()) %>%
  ungroup() -> orders_days
